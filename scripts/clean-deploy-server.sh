...
# This script performs a complete clean deployment on the server

set -e  # Exit on error

SERVER="root@72.62.170.11"
APP_DIR="/root/tool-thinker"

echo "🧹 Starting clean server deployment..."
echo ""

# SSH into server and perform clean deployment
ssh $SERVER << 'ENDSSH'
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "❌ Server deployment failed. Check the output above for errors."
    exit 1
fi
